...
export function AdminRequestOptionalDocumentsSwagger(): MethodDecorator {
  return applyDecorators(
    ...
    ApiOperation({
      ...
      description:
        'Sets requiestOptionalDocuments on the vehicle. When flipped from false to true, emails the driver to upload a permission letter and vehicle schedule. Admin must set it back to false explicitly when the optional document request is no longer needed.',
    }),
    ApiParam({ name: 'vehicleId', type: String, description: 'Vehicle ID' }),
